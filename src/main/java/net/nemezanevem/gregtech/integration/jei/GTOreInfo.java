package net.nemezanevem.gregtech.integration.jei;
...
public class GTOreInfo {
    ...
    public GTOreInfo(OreDepositDefinition definition) {
        ...
        if (ModList.get().isLoaded(MODID_CC)) {
    ...
    public List<ItemStack> findComponentBlocksAsItemStacks() {
        Collection<BlockState> containedStates = new ArrayList<>();
    ...
    private Collection<BlockState> getPossibleStates(FillerEntry entry, Collection<BlockState> collection) {
        for (BlockState state : entry.getPossibleResults()) {
    ...
    private List<ItemStack> getStacksFromStates(Collection<BlockState> states, List<ItemStack> list) {
        for (BlockState state : states) {
            list.add(Util.toItem(state));
        }
        return list;
    }
    ...
    public void addTooltip(int slotIndex, boolean input, Object ingredient, List<Component> tooltip) {
        ...
        //Surface Indicator slot
            ...
            if (veinPopulator instanceof SurfaceRockPopulator) {
                tooltip.add(Component.translatable("gregtech.jei.ore.surface_rock_1"));
                tooltip.add(Component.translatable("gregtech.jei.ore.surface_rock_2"));
            }
        } else {
            if (blockFiller instanceof LayeredBlockFiller) {
    ...
    public List<String> createBiomeTooltip() {
        ...
        for (Map.Entry<Biome, Integer> entry : modifiedBiomeMap.entrySet()) {
            ...
            if (!(entry.getValue() == weight)) {
                //Cannot Spawn
                if (entry.getValue() <= 0) {
                    tooltip.add(Component.translatable("gregtech.jei.ore.biome_weighting_no_spawn", entry.getKey().getBiomeName()));
                } else {
                    tooltip.add(Component.translatable("gregtech.jei.ore.biome_weighting", entry.getKey().getBiomeName(), entry.getValue()));
                }
            }
        }
    ...
    public List<String> createOreWeightingTooltip(int slotIndex) {
        ...
        for (FillerEntry entry : fillerEntries) {
            if (entry.getEntries() != null && !entry.getEntries().isEmpty()) {
                ...
                tooltip.add(Component.translatable("gregtech.jei.ore.ore_weight", weight));
            }
        }
    ...
    private List<String> createOreLayeringTooltip(int slotIndex) {
        ...
        switch (slotIndex) {
            ...
            case 2: {
                tooltip.add(Component.translatable("gregtech.jei.ore.primary_1"));
                tooltip.add(Component.translatable("gregtech.jei.ore.primary_2", filler.getPrimaryLayers()));
                break;
            }
            case 3: {
                tooltip.add(Component.translatable("gregtech.jei.ore.secondary_1"));
                tooltip.add(Component.translatable("gregtech.jei.ore.secondary_2", filler.getSecondaryLayers()));
                break;
            }
            case 4: {
                tooltip.add(Component.translatable("gregtech.jei.ore.between_1"));
                tooltip.add(Component.translatable("gregtech.jei.ore.between_2", filler.getBetweenLayers()));
                break;
            }
            case 5: {
                tooltip.add(Component.translatable("gregtech.jei.ore.sporadic_1"));
                tooltip.add(Component.translatable("gregtech.jei.ore.sporadic_2"));
                break;
            }
        }
        return tooltip;
