package net.nemezanevem.gregtech.module;
...
import net.nemezanevem.gregtech.api.module.*;
...
public class ModuleManager implements IModuleManager {
    ...
    private static final Type MODULE = Type.getType(GregTechModule.class);
    ...
    public void processIMC(ImmutableList<InterModComms.IMCMessage> messages) {
    ...
    private Map<String, List<IGregTechModule>> getModules(ModFileScanData.AnnotationData table) {
    ...
    private List<IGregTechModule> getInstances(ModFileScanData.AnnotationData table) {
        final List<ModFileScanData.AnnotationData> dataSet = ModList.get().getAllScanData().stream()
                .map(ModFileScanData::getAnnotations)
                .flatMap(Collection::stream)
                .filter(a -> MODULE.equals(a.annotationType()))
                .toList();

        List<IGregTechModule> instances = new ArrayList<>();
        for (ModFileScanData.AnnotationData data : dataSet) {
            try {
                Class<?> clazz = Class.forName(data.getClass().getName(), true, IGregTechModule.class.getClassLoader());
                instances.add((IGregTechModule) clazz.newInstance());
            } catch (ClassNotFoundException | IllegalAccessException | InstantiationException e) {
                logger.error("Could not initialize module " + data.annotationData().get("moduleID"), e);
            }
        }
        return instances;
    }
