...
public class MetaTileEntityDistillationTower extends RecipeTypeMultiblockController {

    public MetaTileEntityDistillationTower(ResourceLocation metaTileEntityId) {
        super(metaTileEntityId, RecipeTypes.DISTILLATION_RECIPES);
    }
    ...
    protected void addDisplayText(List<Component> textList) {
        if (isStructureFormed()) {
            ...
            if (stackInTank != null && stackInTank.amount > 0) {
                TextComponentTranslation fluidName = Component.translatable(stackInTank.getFluid().getUnlocalizedName(stackInTank));
                textList.add(Component.translatable("gregtech.multiblock.distillation_tower.distilling_fluid", fluidName));
            }
        }
