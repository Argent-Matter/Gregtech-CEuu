...
public class MetaTileEntityFusionReactor extends RecipeTypeMultiblockController implements IFastRenderMetaTileEntity {
    ...
    public MetaTileEntityFusionReactor(ResourceLocation metaTileEntityId, int tier) {
        super(metaTileEntityId, RecipeTypes.FUSION_RECIPES);
    ...
    public List<MultiblockShapeInfo> getMatchingShapes() {
        ...
        MultiblockShapeInfo.Builder baseBuilder = MultiblockShapeInfo.builder()
                ...
                .aisle("######DCD######", "####GG###GG####", "######UCU######")
                .aisle("###############", "######EME######", "###############")
                .where('M', MetaTileEntities.FUSION_REACTOR[tier - GTValues.LuV], Direction.SOUTH)
                .where('C', getCasingState())
                .where('G', MetaBlocks.TRANSPARENT_CASING.getState(
                        BlockGlassCasing.CasingType.FUSION_GLASS))
                .where('K', getCoilState())
                .where('W', MetaTileEntities.FLUID_EXPORT_HATCH[tier], Direction.NORTH)
                .where('E', MetaTileEntities.FLUID_EXPORT_HATCH[tier], Direction.SOUTH)
                .where('S', MetaTileEntities.FLUID_EXPORT_HATCH[tier], Direction.EAST)
                .where('N', MetaTileEntities.FLUID_EXPORT_HATCH[tier], Direction.WEST)
                .where('w', MetaTileEntities.ENERGY_INPUT_HATCH[tier], Direction.WEST)
                .where('e', MetaTileEntities.ENERGY_INPUT_HATCH[tier], Direction.SOUTH)
                .where('s', MetaTileEntities.ENERGY_INPUT_HATCH[tier], Direction.EAST)
                .where('n', MetaTileEntities.ENERGY_INPUT_HATCH[tier], Direction.NORTH)
                .where('U', MetaTileEntities.FLUID_IMPORT_HATCH[tier], Direction.UP)
                .where('D', MetaTileEntities.FLUID_IMPORT_HATCH[tier], Direction.DOWN)
    ...
    private BlockState getCoilState() {
        if (tier == GTValues.LuV)
    ...
    private void readColor(FriendlyByteBuf buf) {
        color = buf.readBoolean() ? buf.readVarInt() : null;
    }

    private void writeColor(FriendlyByteBuf buf) {
    ...
    protected void addDisplayText(List<Component> textList) {
        ...
        textList.add(Component.translatable("gregtech.multiblock.fusion_reactor.energy", this.energyContainer.getEnergyStored(), this.energyContainer.getEnergyCapacity()));
        textList.add(Component.translatable("gregtech.multiblock.fusion_reactor.heat", heat));
    }

    @Override
    public void addInformation(ItemStack stack, @Nullable World player, List<Component> tooltip, boolean advanced) {
        super.addInformation(stack, player, tooltip, advanced);
        tooltip.add(Component.translatable("gregtech.machine.fusion_reactor.capacity", calculateEnergyStorageFactor(16) / 1000000L));
        tooltip.add(Component.translatable("gregtech.machine.fusion_reactor.overclocking"));
    }
    ...
    public AABB getRenderBoundingBox() {
        return new AABB(this.getPos().offset(getFrontFacing().getOpposite()).offset(getFrontFacing().rotateY(),6),
