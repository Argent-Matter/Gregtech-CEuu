...
public class MetaTileEntityCrate extends MetaTileEntity {
    ...
    public void renderMetaTileEntity(CCRenderState renderState, Matrix4 translation, IVertexOperation[] pipeline) {
        if (material.toString().contains("wood")) {
            Textures.WOODEN_CRATE.render(renderState, translation, Util.convertRGBtoOpaqueRGBA_CL(getPaintingColorForRendering()), pipeline);
        } else {
            int baseColor = ColourRGBA.multiply(Util.convertRGBtoOpaqueRGBA_CL(material.getMaterialRGB()), Util.convertRGBtoOpaqueRGBA_CL(getPaintingColorForRendering()));
