...
public class MetaTileEntityEnergyHatch extends MetaTileEntityMultiblockPart implements IMultiblockAbilityPart<IEnergyContainer> {
    ...
    public void addInformation(ItemStack stack, @Nullable World player, List<Component> tooltip, boolean advanced) {
        ...
        if (isExportHatch) {
            if (amperage > 2) {
                tooltip.add(Component.translatable("gregtech.machine.energy_hatch.output_hi_amp.tooltip"));
            } else {
                tooltip.add(Component.translatable("gregtech.machine.energy_hatch.output.tooltip"));
            }
            tooltip.add(Component.translatable("gregtech.universal.tooltip.voltage_out", energyContainer.getOutputVoltage(), tierName));
            tooltip.add(Component.translatable("gregtech.universal.tooltip.amperage_out_till", energyContainer.getOutputAmperage()));
        } else {
            if (amperage > 2) {
                tooltip.add(Component.translatable("gregtech.machine.energy_hatch.input_hi_amp.tooltip"));
            } else {
                tooltip.add(Component.translatable("gregtech.machine.energy_hatch.input.tooltip"));
            }
            tooltip.add(Component.translatable("gregtech.universal.tooltip.voltage_in", energyContainer.getInputVoltage(), tierName));
