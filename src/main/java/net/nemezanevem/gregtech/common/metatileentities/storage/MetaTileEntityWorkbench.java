...
public class MetaTileEntityWorkbench extends MetaTileEntity implements ICraftingStorage {
    ...
    private final ArrayList<Player> listeners = new ArrayList<>();
    ...
    public void renderMetaTileEntity(CCRenderState renderState, Matrix4 translation, IVertexOperation[] pipeline) {
        ...
        pipeline = ArrayUtils.add(pipeline, new ColourMultiplier(Util.convertRGBtoOpaqueRGBA_CL(paintingColor)));
    ...
    public void addInformation(ItemStack stack, @Nullable Level world, List<Component> tooltip, boolean advanced) {
        tooltip.add(Component.translatable("gregtech.machine.workbench.tooltip1"));
        tooltip.add(Component.translatable("gregtech.machine.workbench.tooltip2"));
    }

    public void discardRecipeResolver(Player entityPlayer) {
        this.listeners.remove(entityPlayer);
        if (listeners.isEmpty()) {
            if (!getWorld().isClientSide && recipeLogic != null) {
