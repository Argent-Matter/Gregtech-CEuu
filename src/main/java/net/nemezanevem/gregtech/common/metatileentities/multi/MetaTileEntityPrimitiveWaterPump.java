...
public class MetaTileEntityPrimitiveWaterPump extends MultiblockControllerBase implements IPrimitivePump {
    ...
    public void tick() {
        super.tick();
        if (!getWorld().isClientSide && getOffsetTimer() % 20 == 0 && isStructureFormed()) {
            if (biomeModifier == 0) {
    ...
    public String[] getDescription() {
        return Stream.of(
                new String[]{Component.translatable("gregtech.multiblock.primitive_water_pump.description")},
                Component.translatable("gregtech.multiblock.primitive_water_pump.extra1").split("/n"),
                Component.translatable("gregtech.multiblock.primitive_water_pump.extra2").split("/n")
