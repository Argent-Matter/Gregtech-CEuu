...
public class MetaTileEntityLockedSafe extends MetaTileEntity implements IFastRenderMetaTileEntity {
    ...
    private final ItemStackHandler unlockInventory = new ItemStackHandler(2) {
        ...
        public ItemStack insertItem(int slot, @Nonnull ItemStack stack, boolean simulate) {
            ...
            return totalAmount == 0 ? ItemStack.EMPTY : Util.copyAmount(totalAmount, stack);
        }
    ...
    private void generateUnlockComponents() {
        if (!getWorld().isClientSide && unlockComponentsSeed == 0L) {
    ...
    private void recheckUnlockItemsAndUnlock() {
        if (getWorld() != null && !getWorld().isClientSide && !isSafeUnlocked() &&
    ...
    private void updateOpenProgress() {
        if (!getWorld().isClientSide && unlockProgress >= 0 && unlockProgress < MAX_UNLOCK_PROGRESS) {
            ++this.unlockProgress;
    ...
    public void renderMetaTileEntityFast(CCRenderState renderState, Matrix4 translation, float partialTicks) {
        ColourMultiplier colourMultiplier = new ColourMultiplier(Util.convertRGBtoOpaqueRGBA_CL(GTValues.VC[unlockComponentTier]));
    ...
    public AABB getRenderBoundingBox() {
        return new AABB(getPos().add(-1, 0, -1), getPos().add(2, 1, 2));
    }
