...
public class FactoryBlockPattern {
    ...
    private TraceabilityPredicate[][][] makePredicateArray() {
        ...
        for (int z = 0; z < this.depth.size(); ++z) {
            for (int y = 0; y < this.aisleHeight; ++y) {
                for (int x = 0; x < this.rowWidth; ++x) {
                    predicate[z][y][x] = this.symbolMap.get(this.depth.get(z)[y].charAt(x));
                }
            }
        }
