...
public class GtRecipes {
    ...
    public static final RegistryObject<RecipeSerializer<SimpleMachineRecipe>> SIMPLE_SERIALIZER = RECIPE_SERIALIZERS.register("simple_serializer", SimpleMachineRecipe.Serializer::new);

    public void registerRecipeTypes(IEventBus bus) {
        RECIPE_TYPES.register(bus);
        RECIPE_SERIALIZERS.register(bus);
        for (var machineId : MachineRegistry.MACHINES_BUILTIN.get().getKeys()) {
            RECIPE_TYPES.register(machineId.path, () -> new RecipeType<SimpleMachineRecipe>() {
                @Override
