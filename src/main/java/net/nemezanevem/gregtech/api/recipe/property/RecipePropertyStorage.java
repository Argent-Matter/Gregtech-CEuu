...
public class RecipePropertyStorage implements IRecipePropertyStorage {
    ...
    public boolean store(RecipeProperty<?> recipeProperty, Object value) {
        ...
        if (frozen) {
            GregTech.LOGGER.warn("Unable to add RecipeProperty with key {} as the storage is frozen", key);
            success = false;
        }
        for (RecipeProperty<?> existingRecipeProperty : recipeProperties.keySet()) {
            if (existingRecipeProperty.getKey().equals(key)) {
                GregTech.LOGGER.warn("Unable to add RecipeProperty with key {} as it already exists", key);
                success = false;
            }
        }

        if (value == null) {
            GregTech.LOGGER.warn("Provided value is null for RecipeProperty with key {}", key);
            success = false;
        }
        ...
        try {
            ...
            GregTech.LOGGER.warn("Provided incorrect value for RecipeProperty with key {}", key);
            GregTech.LOGGER.warn("Full exception:", ex);
            success = false;
        }
    ...
    public void toNetwork(FriendlyByteBuf buffer) {
        buffer.writeBoolean(false); // is NOT empty

        buffer.writeVarInt(this.recipeProperties.size());
        for (var property : this.recipeProperties.entrySet()) {
            buffer.writeRegistryId(RecipePropertyRegistry.RECIPE_PROPERTIES_BUILTIN.get(), property.getKey());
            property.getKey().writeToNetwork(buffer, property.getValue());
        }
    }
    ...
    public void toJson(JsonObject json) {
        ...
        for (var property : this.recipeProperties.entrySet()) {
            ...
            property.getKey().writeToJson(object, property.getValue());
            array.add(object);
        }
    ...
    private RecipeProperty<?> getRecipePropertyValue(String key) {
        ...
        GregTech.LOGGER.warn("There is no property with key {}", key);
        GregTech.LOGGER.warn(STACKTRACE, new IllegalArgumentException());
