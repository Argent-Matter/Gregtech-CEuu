...
import net.nemezanevem.gregtech.api.util.OverlayedItemHandler;
...
public abstract class ParallelLogic {
    ...
    public static int getMaxRecipeMultiplier(@Nonnull GTRecipe recipe, @Nonnull IItemHandlerModifiable inputs, @Nonnull IMultipleTankHandler fluidInputs, int parallelAmount) {
     ...
     */
    public static int limitByOutputMerging(@Nonnull GTRecipe recipe, @Nonnull IItemHandlerModifiable outputs, @Nonnull IMultipleTankHandler fluidOutputs, int parallelAmount, boolean voidItems, boolean voidFluids) {
     ...
     *                   see {@link ParallelLogic#getMaxRecipeMultiplier(GTRecipe, IItemHandlerModifiable, IMultipleTankHandler, int)}
     * @return the amount of times a {@link GTRecipe} outputs can be merged into an inventory without
     * voiding products.
     */
    public static int limitParallelByItems(@Nonnull GTRecipe recipe, @Nonnull OverlayedItemHandler overlayedItemHandler, int multiplier) {
        int minMultiplier = 0;
     ...
     *                   see {@link ParallelLogic#getMaxRecipeMultiplier(GTRecipe, IItemHandlerModifiable, IMultipleTankHandler, int)}
     * @return the amount of times a {@link GTRecipe} outputs can be merged into a fluid handler without
     * voiding products.
     */
    public static int limitParallelByFluids(@Nonnull GTRecipe recipe, @Nonnull OverlayedFluidHandler overlayedFluidHandler, int multiplier) {
        int minMultiplier = 0;
        int maxMultiplier = multiplier;
     ...
     */
    protected static int getMaxRatioItem(@Nonnull Map<ItemStackKey, Integer> countIngredients, @Nonnull GTRecipe recipe, int parallelAmount) {
        ...
        Object2IntOpenHashMap<ExtendedIngredient> notConsumableMap = new Object2IntOpenHashMap<>();
        Object2IntOpenHashMap<ExtendedIngredient> countableMap = new Object2IntOpenHashMap<>();
        for (ExtendedIngredient recipeIngredient : recipe.getInputs()) {

            int ingredientCount = recipeIngredient.values.length;
            if (!recipeIngredient.isConsumable()) {
        ...
        for (Map.Entry<ExtendedIngredient, Integer> recipeInputEntry : notConsumableMap.entrySet()) {
            ...
            for (Map.Entry<ItemStackKey, Integer> inventoryEntry : countIngredients.entrySet()) {
                if (recipeInputEntry.getKey().test(inventoryEntry.getKey().getItemStackRaw())) {
        ...
        for (Map.Entry<ExtendedIngredient, Integer> recipeInputEntry : countableMap.entrySet()) {
            int needed = recipeInputEntry.getValue();
            int available = 0;
            // For every stack in the ingredients gathered from the input bus.
            for (Map.Entry<ItemStackKey, Integer> inventoryEntry : countIngredients.entrySet()) {
     ...
     */
    protected static int getMaxRatioFluid(@Nonnull Map<FluidKey, Integer> countFluid, @Nonnull GTRecipe recipe, int parallelAmount) {
        int minMultiplier = Integer.MAX_VALUE;
        ...
        for (FluidIngredient fluidInput : recipe.getFluidInputs()) {
            ...
            if (!fluidInput.isConsumable()) {
                notConsumableMap.computeIfPresent(new FluidKey(fluidInput.getFluids()[0]), (k, v) -> v + fluidAmount);
                notConsumableMap.putIfAbsent(new FluidKey(fluidInput.getFluids()[0]), fluidAmount);
            } else {
                fluidCountMap.computeIfPresent(new FluidKey(fluidInput.getFluids()[0]), (k, v) -> v + fluidAmount);
    ...
    public static GTRecipeBuilder<?> doParallelRecipes(@Nonnull GTRecipe currentRecipe, @Nonnull GTRecipeType<?> recipeMap, @Nonnull IItemHandlerModifiable importInventory, @Nonnull IMultipleTankHandler importFluids, @Nonnull IItemHandlerModifiable exportInventory, @Nonnull IMultipleTankHandler exportFluids, int parallelAmount, long maxVoltage, @Nonnull IVoidable voidable) {
        ...
        GTRecipeBuilder<?> recipeBuilder = recipeMap.recipeBuilder();
    ...
    /**
     * Constructs a {@link GTRecipeBuilder} containing the recipes from the ItemStacks available in the {@code importInventory}
     * Does NOT take fluids into account whatsoever
     *
     * @param recipeMap       The {@link GTRecipeType} to search for recipes
    ...
    public static GTRecipeBuilder<?> appendItemRecipes(@Nonnull GTRecipeType<?> recipeMap, @Nonnull IItemHandlerModifiable importInventory, @Nonnull IItemHandlerModifiable exportInventory, int parallelAmount, long maxVoltage, IVoidable voidable) {
        ...
        for (int index = 0; index < importInventory.getSlots(); index++) {
            ...
            GTRecipe matchingRecipe = recipeMap.findRecipe(maxVoltage,
                    Collections.singletonList(currentInputItem),
                    Collections.emptyList(), 0);

            ExtendedIngredient inputIngredient;
            if (matchingRecipe != null) {
                ...
                if (recipeBuilder == null) {
                    ...
                    recipeBuilder = recipeMap.recipeBuilder().setEUt(0).setBaseDuration(0);
            ...
            int ingredientRatio = Math.min(parallelAmount - engagedItems, currentInputItem.getCount() / Math.max(matchingRecipe.getInputs().get(0).getItems()[0].getCount(), 1));
