...
public class WireProperty implements IMaterialProperty<WireProperty> {
    ...
    public WireProperty(int voltage, int baseAmperage, int lossPerBlock, boolean isSuperCon) {
        this(voltage, baseAmperage, lossPerBlock, isSuperCon, 0);
    }

    public WireProperty(int voltage, int baseAmperage, int lossPerBlock, boolean isSuperCon, int criticalTemperature) {
    ...
    public void verifyProperty(MaterialProperties properties) {
        ...
        if (properties.hasProperty(GtMaterialProperties.INGOT.get())) {
            ...
            if (!isSuperconductor && voltage >= GTValues.V[GTValues.IV] && !thisMaterial.hasFlag(GtMaterialFlags.GENERATE_FOIL.get())) {
                thisMaterial.addFlags(GtMaterialFlags.GENERATE_FOIL.get());
            }
        }
    }
