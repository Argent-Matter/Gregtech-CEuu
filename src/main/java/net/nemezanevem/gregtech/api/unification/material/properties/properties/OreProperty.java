...
public class OreProperty implements IMaterialProperty<OreProperty> {
    ...
    public void verifyProperty(MaterialProperties properties) {
        ...
        if (directSmeltResult != null) directSmeltResult.getProperties().ensureSet(GtMaterialProperties.DUST.get(), true);
        if (washedIn != null) washedIn.getProperties().ensureSet(GtMaterialProperties.FLUID.get(), true);
        separatedInto.forEach(m -> m.getProperties().ensureSet(GtMaterialProperties.DUST.get(), true));
        oreByProducts.forEach(m -> m.getProperties().ensureSet(GtMaterialProperties.DUST.get(), true));
    }
}